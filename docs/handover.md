...
## 7. 미해결
운영 홈(`/ops/`)이 이 절을 그대로 읽어 보여준다. 해결되면 여기서 지운다.

- **업무 범위 문서가 확정되지 않았다.** 근로계약서 제2조② 12항목이 전부 「보조·지원」인데, 실제로 들어오는 요청은 그 경계에 걸치는 것이 있다. 문서로 못 박아 두어야 다음 사람이 곤란해지지 않는다.
- **계약 200시간 대 시간표 249시간.** 시간표대로 근무하면 계약 시간을 넘긴다. 정산 방식을 정해야 한다. 근거 숫자는 운영 홈의 「이번 달 근무 시간」에서 나온다.
- **빌린 계정의 범위.** 아이모두 교사 계정을 빌려 쓰고 있다. 어디까지 써도 되는지, 언제까지인지 합의된 문서가 없다.
